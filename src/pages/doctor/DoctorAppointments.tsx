...
export default function DoctorAppointments() {
  ...
  const { data: appointments, isLoading } = useQuery({ queryKey: ["doctor-appointments-list", doctorRecord?.id], queryFn: async () => { const { data, error } = await supabase.from("appointments").select(`*, profiles!appointments_patient_profile_fkey (full_name, avatar_url, email, phone)`).eq("doctor_id", doctorRecord!.id).order("appointment_date", { ascending: false }); if (error) throw error; return data; }, enabled: !!doctorRecord });
  const { data: notes } = useQuery({ queryKey: ["appointment-notes", detailId], queryFn: async () => { const { data, error } = await supabase.from("appointment_notes").select("*").eq("appointment_id", detailId!).order("created_at", { ascending: true }); if (error) throw error; return data; }, enabled: !!detailId });

  const updateStatus = useMutation({ mutationFn: async ({ id, status }: { id: string; status: string }) => { const { error } = await supabase.from("appointments").update({ status: status as any }).eq("id", id); if (error) throw error; }, onSuccess: () => { toast.success(t.appointmentUpdated); queryClient.invalidateQueries({ queryKey: ["doctor-appointments-list"] }); queryClient.invalidateQueries({ queryKey: ["doctor-all-appointments"] }); }, onError: (err: any) => toast.error(err.message) });
  const addNote = useMutation({ mutationFn: async () => { if (!user || !detailId || !clinicalNote.trim()) throw new Error("Missing data"); const { error } = await supabase.from("appointment_notes").insert({ appointment_id: detailId, content: clinicalNote.trim(), created_by: user.id }); if (error) throw error; }, onSuccess: () => { toast.success(t.noteAdded); setClinicalNote(""); queryClient.invalidateQueries({ queryKey: ["appointment-notes"] }); }, onError: (err: any) => toast.error(err.message) });
          ...
          <motion.div key={apt.id} className="glass rounded-2xl p-5 shadow-card hover:shadow-elevated transition-all cursor-pointer" custom={i} variants={fadeUp} initial="hidden" animate="visible" onClick={() => { setDetailId(apt.id); setClinicalNote(""); }}>
            <div className="flex items-center justify-between gap-4">
              ...
              <div className="flex items-center gap-2 flex-shrink-0">
                {apt.status === "pending" && (<><Button size="icon" variant="ghost" className="h-8 w-8 text-success hover:text-success hover:bg-success/10" onClick={(e) => { e.stopPropagation(); updateStatus.mutate({ id: apt.id, status: "confirmed" }); }}><Check className="h-4 w-4" /></Button><Button size="icon" variant="ghost" className="h-8 w-8 text-destructive hover:text-destructive hover:bg-destructive/10" onClick={(e) => { e.stopPropagation(); updateStatus.mutate({ id: apt.id, status: "cancelled" }); }}><X className="h-4 w-4" /></Button></>)}
        ...
        <Tabs defaultValue="pending">
          <motion.div custom={1} variants={fadeUp}>
            <TabsList className="rounded-xl mb-6">
              <TabsTrigger value="pending" className="rounded-lg">{t.pending} {pending.length > 0 && <Badge className="ml-1.5 h-5 min-w-5 rounded-full bg-warning text-warning-foreground text-xs">{pending.length}</Badge>}</TabsTrigger>
        ...
        <DialogContent className="sm:max-w-lg rounded-2xl max-h-[85vh] overflow-y-auto">
          {selectedAppointment && (() => { const patient = selectedAppointment.profiles as any; const status = statusConfig[selectedAppointment.status]; const StatusIcon = status.icon; return (<>
            ...
            <div className="space-y-4">
              ...
              {selectedAppointment.status === "pending" && <div className="grid grid-cols-2 gap-3"><Button className="rounded-xl bg-success hover:bg-success/90 text-success-foreground" onClick={() => updateStatus.mutate({ id: selectedAppointment.id, status: "confirmed" })} disabled={updateStatus.isPending}><Check className="mr-2 h-4 w-4" /> {t.confirm}</Button><Button variant="destructive" className="rounded-xl" onClick={() => updateStatus.mutate({ id: selectedAppointment.id, status: "cancelled" })} disabled={updateStatus.isPending}><X className="mr-2 h-4 w-4" /> {t.decline}</Button></div>}
