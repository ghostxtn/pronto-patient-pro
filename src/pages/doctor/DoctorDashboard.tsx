...
import {
  CalendarCheck, Clock, Users, Activity, ArrowRight, AlertCircle, CheckCircle2,
} from "lucide-react";
...
export default function DoctorDashboard() {
  ...
  const stats = [
    { icon: CalendarCheck, label: "Today", value: String(todayAppts.length), color: "from-primary to-info" },
    { icon: AlertCircle, label: "Pending", value: String(pending.length), color: "from-warning to-destructive" },
    { icon: CheckCircle2, label: "Confirmed", value: String(confirmed.length), color: "from-secondary to-success" },
      ...
      <motion.div initial="hidden" animate="visible">
        ...
        <motion.div className="glass rounded-2xl p-6 shadow-card mb-6" custom={6} variants={fadeUp}>
          <div className="flex items-center justify-between mb-4">
            ...
            <Button variant="ghost" size="sm" asChild>
              <Link to="/doctor/appointments">View All <ArrowRight className="ml-1 h-3 w-3" /></Link>
            </Button>
          </div>
          {todayAppts.length > 0 ? (
                ...
                return (
                  <div key={apt.id} className="flex items-center justify-between p-3 rounded-xl bg-muted/50">
                    <div className="flex items-center gap-3">
                      <div className="h-10 w-10 rounded-xl bg-gradient-to-br from-secondary to-success flex items-center justify-center flex-shrink-0">
                        <span className="text-secondary-foreground font-display font-bold text-sm">
                          {patient?.full_name?.[0] || "P"}
                        </span>
                      </div>
                      <div>
                        <div className="font-medium text-sm">{patient?.full_name || "Patient"}</div>
                    ...
                    <Badge className={cn("rounded-full border", statusColors[apt.status])} variant="outline">
                      {apt.status}
                    </Badge>
                  </div>
                );
        ...
        <motion.div className="grid md:grid-cols-2 gap-4" custom={7} variants={fadeUp}>
          <Link to="/doctor/schedule" className="glass rounded-2xl p-6 shadow-card hover:shadow-elevated transition-all group">
            ...
            <h3 className="font-display font-semibold mb-1">Manage Schedule</h3>
            <p className="text-sm text-muted-foreground">Set your weekly availability and working hours.</p>
          </Link>
          <Link to="/doctor/appointments" className="glass rounded-2xl p-6 shadow-card hover:shadow-elevated transition-all group">
            ...
            <h3 className="font-display font-semibold mb-1">View Appointments</h3>
            <p className="text-sm text-muted-foreground">Review, confirm, and manage patient appointments.</p>
          </Link>
        </motion.div>
      </motion.div>
    </AppLayout>
