...
const statusConfig: Record<string, { color: string; icon: React.ElementType; label: string }> = {
  pending: { color: "bg-warning/15 text-warning border-warning/30", icon: Clock, label: "Bekliyor" },
  confirmed: { color: "bg-primary/15 text-primary border-primary/30", icon: CheckCircle2, label: "Onaylandi" },
  completed: { color: "bg-success/15 text-success border-success/30", icon: CheckCircle2, label: "Tamamlandi" },
  cancelled: { color: "bg-destructive/15 text-destructive border-destructive/30", icon: XCircle, label: "Iptal" },
  ...
  return (
    <AppLayout>
      <motion.div initial="hidden" animate="visible" className="space-y-6">
        <motion.div custom={0} variants={fadeUp}><h1 className="text-3xl font-display font-bold">{t.manageAppointments}</h1><p className="text-muted-foreground mt-1">{appointments?.length ?? 0} {t.totalAppointments}</p></motion.div>
