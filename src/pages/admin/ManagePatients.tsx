    ...
    <AppLayout>
      <motion.div initial="hidden" animate="visible" className="space-y-6">
        ...
        <motion.div custom={2} variants={fadeUp}>
          ...
          <tbody>{filtered.map((p) => (<tr key={p.id} className="border-b last:border-0 hover:bg-muted/40 transition-colors"><td className="p-4"><div className="flex items-center gap-3"><div className="h-9 w-9 rounded-full bg-gradient-to-br from-secondary to-success flex items-center justify-center text-primary-foreground font-bold text-sm">{p.full_name?.[0] ?? "P"}</div><p className="font-medium text-sm">{p.full_name ?? "Unknown"}</p></div></td><td className="p-4 hidden md:table-cell text-sm text-muted-foreground">{p.email ?? "—"}</td><td className="p-4 hidden lg:table-cell text-sm text-muted-foreground">{p.phone ?? "—"}</td><td className="p-4 hidden lg:table-cell text-sm text-muted-foreground">{format(new Date(p.created_at), "MMM d, yyyy")}</td></tr>))}{!filtered.length && <tr><td colSpan={4} className="p-8 text-center text-muted-foreground">—</td></tr>}</tbody></table></div></CardContent></Card>
        </motion.div>
      </motion.div>
    </AppLayout>
  );
